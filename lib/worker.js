...
async function handleJobs (queue, jobIdsToHandle) {
  ...
  let jobDatas = []
  let deletedErrors = []
  let retryLimits = { jobIds: [], datas: [] }
  let invalidData = { jobIds: [], datas: [] }
  let errs = []
  ...
  for (let i = 0; i < jobIdsToHandle.length; i++) {
    ...
    if (!jobState.data || jobState.deletedAt) {
      cleanUpJobs.push([jobId, 'dataDeletedError'])
      deletedErrors.push(jobId)
      continue
    }

    if ((Number(jobState.attempts) || 0) >= (Number(jobState.maxAttempts) || this.maxAttempts)) {
      cleanUpJobs.push([jobId, 'maxAttempts'])
      retryLimits.jobIds.push(jobId)
      retryLimits.datas.push(jobState.data)
      continue
    }

    let jobData
    try {
      jobData = JSON.parse(jobState.data)
    } catch (err) {
      cleanUpJobs.push([jobId, 'dataInvalidError'])
      invalidData.jobIds.push(jobIds)
      invalidData.datas.push(jobState.data)
      continue
    }
  ...
  if (retryLimits.jobIds.length) {
    this.emit('retry-limit', queue, retryLimits.jobIds, retryLimits.datas, new Error('Jobs reached retry limit'))
  }

  if (invalidData.jobIds.length) {
    this.emit('invalid-data', queue, invalidData.jobIds, invalidData.datas, new Error('Jobs data could not be decoded'))
  }
  ...
  await this.redisAccess.push(async () => {
    ...
    for (let jobId of jobIds) {
      cmd = cmd
        ...
        .hset(`${this.namespace}:job:${jobId}`, 'workerId', this.id)
    }
    await cmd.exec()
  })

  this.emit('start', queue, jobIds, jobDatas)
  try {
    await this.handler(queue, jobDatas)
  } catch (err) {
    await this.redisAccess.push(async () => {
      let cmd = this.redis.multi()
      for (let jobId of jobIds) {
        cmd = cmd
          ...
          .hset(`${this.namespace}:job:${jobId}`, `errorAt`, Date.now())
      }
      cmd = cmd.rpush(`${this.namespace}:queue:${queue}`, ...[...jobIds].reverse())
      await cmd.exec()
    })
    this.emit('handler-error', queue, jobIds, err, jobDatas)
    return
  }
  await cleanUpActiveJobs.call(this, queue, jobIds.map(jobId => [jobId, 'finish']))
  this.emit('finish', queue, jobIds, jobDatas)
}
...
async function getJobsFromQueue (queue) {
  return this.redisAccess.push(async () => {
    ...
    const results = await cmd.exec()
    if (results === null) { // This is the case when the watch causes the multi to fail
      return [false]
    }
...
async function shouldHandleActive (queue, jobIds) {
  ...
  for (let [errT, timeout] of results) {
    throwErrs(errT)
    totalTimeout += (Number(timeout) || 0)
  }

  const now = Date.now()
  if (fetchAt && now - Number(fetchAt) < (totalTimeout || this.timeout)) {
    await this.redis.unwatch()
    return false
  }
